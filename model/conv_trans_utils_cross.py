...
class LinearAttentionDecoder(nn.Module):
    ...
    def forward(self, q, x):
        ...
        k, v = map(lambda t: rearrange(t, 'b (dim_head heads) h w -> b heads (h w) dim_head', dim_head=self.dim_head, heads=self.heads, 
                                       h=self.reduce_size, w=self.reduce_size), (k, v))
...
class down_block_cross_trans(nn.Module):
    def __init__(self, in_ch, out_ch, num_block, bottleneck=False, maxpool=True, heads=4, dim_head=64, attn_drop=0., proj_drop=0., reduce_size=16, projection='interp', rel_pos=True):
        ...
        cross_attn_block = BasicTransDecoderBlock
        attn_block = BasicTransBlock
        ...
        if maxpool:
            ...
            block_list.append(block(in_ch, out_ch, stride=2))
        self.blocks = nn.Sequential(*block_list)
        self.cross_attn= cross_attn_block(in_ch, out_ch, heads=4, dim_head=dim_head, attn_drop=attn_drop, proj_drop=proj_drop, reduce_size=reduce_size, projection=projection, rel_pos=rel_pos)
        assert num_block > 0
#        block_list2.append(attn_block(2*out_ch, heads, dim_head, attn_drop=attn_drop, proj_drop=proj_drop, reduce_size=reduce_size, projection=projection, rel_pos=rel_pos))
        self.conv1=conv1x1(2*out_ch,out_ch)
        for i in range(num_block):
            block_list2.append(attn_block(out_ch, heads, dim_head, attn_drop=attn_drop, proj_drop=proj_drop, reduce_size=reduce_size, projection=projection, rel_pos=rel_pos))
        self.blocks2 = nn.Sequential(*block_list2)
...
class up_block_att_trans(nn.Module):
    def __init__(self, in_ch, out_ch, num_block, bottleneck=False, heads=4, dim_head=64, attn_drop=0., proj_drop=0., reduce_size=16, projection='interp', rel_pos=True):
        super().__init__()
        self.conv_ch = nn.Conv2d(in_ch, out_ch, kernel_size=1)
        self.attn_decoder = AttentionBlock(out_ch, out_ch, in_ch//2)
...
class up_block_GSAM(nn.Module):
    def __init__(self, in_ch, out_ch, num_block, bottleneck=False, heads=4, dim_head=64, attn_drop=0., proj_drop=0., reduce_size=16, projection='interp', rel_pos=True):
        ...
        self.conv_ch = nn.Conv2d(in_ch, out_ch, kernel_size=1)
        self.attn_decoder = AttentionBlock(out_ch, out_ch, in_ch//2)
        self.CAB = BasicTransDecoderBlock(in_ch, out_ch, heads=heads, dim_head=dim_head, attn_drop=attn_drop, proj_drop=proj_drop, reduce_size=reduce_size, projection=projection, rel_pos=rel_pos)
    ...
    def forward(self, x1, x2):
        # x1: low-res feature, x2: high-res feature
        x11 = F.interpolate(self.conv_ch(x1), size=x2.shape[-2:], mode='bilinear', align_corners=True)
        out1 = self.CAB(x1, x2)
        out = self.attn_decoder(out1, x11)
        out = torch.cat([out, out1,x11], dim=1)
        out = self.blocks(out)
